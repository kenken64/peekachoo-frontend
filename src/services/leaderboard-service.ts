...
export class LeaderboardService {
	...
	static async getGlobalLeaderboard(
		...
		const response = await fetch(
			`${LeaderboardService.API_URL}/global?${queryParams.toString()}`,
			{
				headers: {
	 ...
	 */
	static async getAroundMe(
		params: { period?: LeaderboardPeriod; range?: number } = {},
		...
		const response = await fetch(
			`${LeaderboardService.API_URL}/around-me?${queryParams.toString()}`,
			{
				headers: {
					Authorization: `Bearer ${token}`,
				},
	...
	static async getLevelLeaderboard(
		...
		const response = await fetch(
			`${LeaderboardService.API_URL}/level/${level}?${queryParams.toString()}`,
			{
				headers: {
					Authorization: `Bearer ${token}`,
				},
		...
		if (!result.success) {
			throw new Error(
				result.error?.message || "Failed to fetch level leaderboard",
			);
		}
	...
	static async getGameLeaderboard(
		...
		const response = await fetch(
			`${LeaderboardService.API_URL}/game/${gameId}?${queryParams.toString()}`,
			{
				headers: {
					Authorization: `Bearer ${token}`,
				},
		...
		if (!result.success) {
			throw new Error(
				result.error?.message || "Failed to fetch game leaderboard",
			);
		}
	...
	static async submitScore(data: {
		...
		const token = AuthService.getToken();
		const response = await fetch(`${LeaderboardService.API_URL}/scores`, {
			method: "POST",
			headers: {
	...
	static async startSession(gameId?: string | null): Promise<string> {
		const token = AuthService.getToken();
		const response = await fetch(`${LeaderboardService.API_URL}/sessions`, {
			method: "POST",
			headers: {
				"Content-Type": "application/json",
				Authorization: `Bearer ${token}`,
	...
	static async endSession(sessionId: string): Promise<{
		...
		const token = AuthService.getToken();
		const response = await fetch(
			`${LeaderboardService.API_URL}/sessions/${sessionId}/end`,
			{
				method: "POST",
				headers: {
					Authorization: `Bearer ${token}`,
