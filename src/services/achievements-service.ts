...
export class AchievementsService {
	...
	static async getAchievements(): Promise<AchievementsResponse> {
		...
		const response = await fetch(AchievementsService.API_URL, {
			headers: {
	 ...
	 */
	static async getAchievement(achievementId: string): Promise<
		Achievement & {
			isUnlocked: boolean;
			unlockedAt?: string;
	...
	> {
		...
		const response = await fetch(
			`${AchievementsService.API_URL}/${achievementId}`,
			{
				headers: {
					Authorization: `Bearer ${token}`,
				},
	...
	static async getAchievementsByCategory(
		...
		const token = AuthService.getToken();
		const response = await fetch(
			`${AchievementsService.API_URL}/category/${category}`,
			{
				headers: {
					Authorization: `Bearer ${token}`,
				},
		...
		if (!result.success) {
			throw new Error(
				result.error?.message || "Failed to fetch category achievements",
			);
		}
	...
	static async getCategories(): Promise<{ categories: CategorySummary[] }> {
		const token = AuthService.getToken();
		const response = await fetch(`${AchievementsService.API_URL}/categories`, {
			headers: {
				Authorization: `Bearer ${token}`,
			},
		});
