...
export class StatsService {
	...
	static async getMyStats(): Promise<PlayerStatsResponse> {
		...
		const response = await fetch(`${StatsService.API_URL}/me`, {
			headers: {
	...
	static async getPlayerStats(userId: string): Promise<PlayerStatsResponse> {
		const token = AuthService.getToken();
		const response = await fetch(`${StatsService.API_URL}/player/${userId}`, {
			headers: {
				Authorization: `Bearer ${token}`,
			},
		});
	...
	static async getGameHistory(
		params: { limit?: number; offset?: number; gameId?: string } = {},
	): Promise<{
		...
		const response = await fetch(
			`${StatsService.API_URL}/history?${queryParams.toString()}`,
			{
				headers: {
					Authorization: `Bearer ${token}`,
				},
	...
	static async getCollection(
		...
		const response = await fetch(
			`${StatsService.API_URL}/collection?${queryParams.toString()}`,
			{
				headers: {
					Authorization: `Bearer ${token}`,
				},
	...
	static calculateSkillBreakdown(stats: PlayerStats): {
		...
		return {
			...
			speed: Math.min(
				100,
				Math.max(0, 100 - ((stats.fastestLevelSeconds || 60) - 15) * 2),
			),
