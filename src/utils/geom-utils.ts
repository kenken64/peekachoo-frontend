...
export class GeomUtils {
	...
	static lineContainsLine(line1: Line, line2: Line): boolean {
		const m1Abs = Math.abs(GeomUtils.calculateSlope(line1));
		const m2Abs = Math.abs(GeomUtils.calculateSlope(line2));
		...
		if (m1Abs === m2Abs) {
			...
			//
				...
				if (
					GeomUtils.calculateYIntercept(line1) !==
					GeomUtils.calculateYIntercept(line2)
				) {
					return false;
				}
	...
	static lineContainsLineAndNotEqual(line1: Line, line2: Line): boolean {
		return (
			!GeomUtils.linesAreEqual(line1, line2) &&
			GeomUtils.lineContainsLine(line1, line2)
		);
	}

	static lineContainsAnyLine(line1: Line, lines: Line[]): boolean {
		return lines.some((line) => GeomUtils.lineContainsLine(line1, line));
	}

	static lineOverlapsAnyLine(line1: Line, lines: Line[]): boolean {
		return lines.some(
			(line) =>
				GeomUtils.lineContainsLine(line1, line) ||
				GeomUtils.lineContainsLine(line, line1),
		);
	}

	static linesContainLine(lines: Line[], line1: Line): boolean {
		return lines.some((line) => GeomUtils.lineContainsLine(line, line1));
	}
	 ...
	 */
	static subtractLinesWhereLine1ContainsLine2(
		line1: Line,
		line2: Line,
	): Line[] {
		...
		if (
			...
			if (line1.y1 === line2.y1) {
				...
				return [
					new Line(x, line1.y1, x, line2.y1),
					new Line(x, line2.y2, x, line1.y2),
				];
			}
		} else if (horizontal) {
			const y = line1.y1;
			if (line1.x1 === line2.x1) {
				...
				return [
					new Line(line1.x1, y, line2.x1, y),
					new Line(line2.x2, y, line1.x2, y),
				];
			}
		}
	}
	...
	static getPolygonPointsFromLines(lines: Line[]): ExtPoint[] {
		const points: ExtPoint[] = lines.map(
			(line) => new ExtPoint(new Point(line.x1, line.y1)),
		);
	...
	static makeClockwisePoints(points: ExtPoint[]): ExtPoint[] {
		if (!GeomUtils.isClockwisePoints(points)) {
			points.reverse();
		}
	...
	static isClockwisePoints(points: ExtPoint[]): boolean {
		return GeomUtils.isClockwiseLines(
			GeomUtils.getLinesFromPolygonPoints(points),
		);
	}
	...
	static reverseLines(lines: Line[]): Line[] {
		const reversedLines: Line[] = lines.map(
			(line) => new Line(line.x2, line.y2, line.x1, line.y1),
		);
	...
	static calculatePointFromOrigin(
		origin: Point,
		angleDegrees: number,
		distance: number,
	): Point {
		const xAdder =
			distance * Math.cos(GeomUtils.degressToRadians(angleDegrees));
		const yAdder =
			distance * Math.sin(GeomUtils.degressToRadians(angleDegrees));
	...
	static collisionLineSegmentArrays(lines1: Line[], lines2: Line[]): boolean {
		for (const line1 of lines1) {
			for (const line2 of lines2) {
				if (GeomUtils.collisionLineSegments(line1, line2)) {
					return true;
				}
			}
		}
	...
	static collisionLineSegments(line1: Line, line2: Line): boolean {
		if (GeomUtils.linesAreEqual(line1, line2)) {
			return true;
		}

		if (
			GeomUtils.lineContainsLine(line1, line2) ||
			GeomUtils.lineContainsLine(line2, line1)
		) {
			return true;
		}

		const m1 = GeomUtils.calculateSlope(line1);
		const m2 = GeomUtils.calculateSlope(line2);

		const vertical1 = m1 === Infinity || m1 === -Infinity;
		const vertical2 = m2 === Infinity || m2 === -Infinity;

		const b1 = GeomUtils.calculateYIntercept(line1);
		const b2 = GeomUtils.calculateYIntercept(line2);
	...
	static calculateYIntercept(line: Line): number {
		const m = GeomUtils.calculateSlope(line);
