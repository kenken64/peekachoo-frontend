...
class SessionStore {
	...
	recordDeath(): void {
		if (this.session?.currentLevelData) {
			...
			logger.log(
				"[SessionStore] Death recorded, lives remaining:",
				this.session.currentLevelData.livesRemaining,
			);
		}
	}
	...
	async completeLevel(): Promise<ScoreSubmissionResult | null> {
		...
		if (!this.session?.currentLevelData) {
			logger.warn(
				"[SessionStore] No active level to complete - currentLevelData is null",
			);
			return null;
		}
		...
		const timeTakenSeconds = Math.floor(
			(levelData.endTime - levelData.startTime) / 1000,
		);
		...
		try {
			...
			const territoryAsDecimal =
				levelData.territoryPercentage > 1
...
export type { SessionData, LevelData };
