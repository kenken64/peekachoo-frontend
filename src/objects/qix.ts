...
export class Qix {
	...
	private getRandomDegrees(degrees: number) {
		let randomDegrees =
			Math.random() * (Qix.MAX_CHANGE_DEG - Qix.MIN_CHANGE_DEG) +
			Qix.MIN_CHANGE_DEG +
			degrees;
	...
	private move(): void {
		...
		const originalLineDegrees: number = this.currentLineDegrees;
		...
		do {
			...
			if (count > 360) {
				logger.info(
					"Houston we have a problem - not sure how to make the qix move again...",
				);
			...
			const nextPoint = GeomUtils.calculatePointFromOrigin(
				this.getPoint(),
			...
			const nextLine = this.getNextLine(
				nextPoint,
				this.currentLineDegrees,
				this.rotationalPointDistance,
				this.currentLineLength,
			);

			collision =
				this.scene.grid.frame.collisionWithLine(nextLine) ||
				this.scene.grid.frame.nonInteresectingLineOutside(nextLine) ||
				this.scene.grid.filledPolygons.pointWithinPolygon(
					new ExtPoint(new Point(nextLine.x1, nextLine.y1)),
				) ||
				this.scene.grid.filledPolygons.pointWithinPolygon(
					new ExtPoint(new Point(nextLine.x2, nextLine.y2)),
				);
			...
			if (collision) {
				if (secondCollisionProcessed) {
					...
					this.directionDegrees =
						(originalLineDegrees + Qix.MIN_CHANGE_DEG) % 360;
		...
		const newPoint = GeomUtils.calculatePointFromOrigin(
			this.getPoint(),
			this.directionDegrees,
			this.speed,
		);
	...
	checkForCollisionWithCurrentLines(): boolean {
		return (
			GeomUtils.collisionLineSegmentArrays(
				this.lines,
				this.scene.grid.currentLines.lines,
			) ||
			(this.scene.grid.currentLines.line != null &&
				GeomUtils.collisionLineSegmentArrays(this.lines, [
					this.scene.grid.currentLines.line,
				]))
		);
	}
	...
	private createNextLine(): Line {
		...
		this.currentLineDegrees =
			(this.currentLineDegrees + this.lineDegreesIncrement) % 360;
		return line;
	}
	...
	private getNextLine(
		point: Point,
		lineDegrees: number,
		rotationalPointDistance: number,
		length: number,
	): Line {
		const tailPoint = GeomUtils.calculatePointFromOrigin(
			point,
			180 + lineDegrees,
			rotationalPointDistance,
		);
		const headPoint = GeomUtils.calculatePointFromOrigin(
			point,
			lineDegrees,
			length - rotationalPointDistance,
		);
