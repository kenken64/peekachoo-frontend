...
export class Sparky {
	...
	move(): void {
		const directions = this.getFilteredPossibleDirections();

		const randomIndex = Math.floor(Math.random() * directions.size);
		const randomDirection = directions.getArray()[randomIndex];
	...
	private getUpPoint(): Point {
		return new Point(this.x, this.y - this.speed);
	}
	private getDownPoint(): Point {
		return new Point(this.x, this.y + this.speed);
	}
	private getLeftPoint(): Point {
		return new Point(this.x - this.speed, this.y);
	}
	private getRightPoint(): Point {
		return new Point(this.x + this.speed, this.y);
	}
	...
	private perpendicularDirections(directions: Set<Direction>): Set<Direction> {
		const perpendicularDirections: Set<Direction> = new Set();

		if (this.direction === Direction.UP || this.direction === Direction.DOWN) {
			if (directions.contains(Direction.LEFT))
				perpendicularDirections.set(Direction.LEFT);
			if (directions.contains(Direction.RIGHT))
				perpendicularDirections.set(Direction.RIGHT);
		} else {
			if (directions.contains(Direction.UP))
				perpendicularDirections.set(Direction.UP);
			if (directions.contains(Direction.DOWN))
				perpendicularDirections.set(Direction.DOWN);
		}
	...
	private getPossibleDirections(): Set<Direction> {
		...
		if (onFrame) {
			...
			if (frame.pointOnOutline(this.getDownPoint()))
				directions.set(Direction.DOWN);
			if (frame.pointOnOutline(this.getLeftPoint()))
				directions.set(Direction.LEFT);
			if (frame.pointOnOutline(this.getRightPoint()))
		...
		const addDirectionsFromLines = (dirs: Set<Direction>, lines: Line[]) => {
			lines.forEach((line: Line) => {
				if (Phaser.Geom.Intersects.PointToLineSegment(this.getUpPoint(), line))
					dirs.set(Direction.UP);
				if (
					Phaser.Geom.Intersects.PointToLineSegment(this.getDownPoint(), line)
				)
					dirs.set(Direction.DOWN);
				if (
					Phaser.Geom.Intersects.PointToLineSegment(this.getLeftPoint(), line)
				)
					dirs.set(Direction.LEFT);
				if (
					Phaser.Geom.Intersects.PointToLineSegment(this.getRightPoint(), line)
				)
	...
	private rand(): integer {
		const rand = Math.floor(Math.random() * Math.floor(this.pointRange));
