...
export class Player {
	...
	constructor(scene: Scene, x: integer, y: integer) {
		...
		this.effectiveRadius = isMobile
			? Math.max(customConfig.playerRadius * 2, 10)
			: customConfig.playerRadius;
	...
	point(): ExtPoint {
		return ExtPoint.createWithCoordinates(
			this.graphics.x + customConfig.playerRadius,
			this.graphics.y + customConfig.playerRadius,
		);
	}
	...
	movingLeft(): boolean {
		return this.x() < this.previousPoint.x();
	}
	movingRight(): boolean {
		return this.x() > this.previousPoint.x();
	}
	movingUp(): boolean {
		return this.y() < this.previousPoint.y();
	}
	movingDown(): boolean {
		return this.y() > this.previousPoint.y();
	}
	movingHoriziontally(): boolean {
	...
	activateSpeedBoost(scene: Scene, durationMs: number = 20000) {
		...
		this.speedBoostTimer = scene.time.delayedCall(
			durationMs,
			() => {
