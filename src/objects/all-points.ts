...
export class AllPoints {
	...
	constructor(scene: QixScene, rectangle: Rectangle) {
		this.scene = scene;
		this.innerPolygonPointsClockwise =
			GeomUtils.getClockwiseRectanglePoints(rectangle);
	}
	...
	calculateNewClockwisePolygonPoints(points: ExtPoint[]): ExtPoint[] {
		...
		if (newPoints.length < 3 && newInversePoints.length < 3) {
			logger.warn(
				"Both newPoints and newInversePoints have fewer than 3 points, returning empty array",
			);
			return [];
		}
		...
		if (newPoints.length < 3) {
			...
			const newClockwisePoints =
				GeomUtils.makeClockwisePoints(newInversePoints);
		...
		if (newInversePoints.length < 3) {
			...
			const newClockwisePoints = GeomUtils.makeClockwisePoints(newPoints);
			newClockwisePoints.splice(newClockwisePoints.length - 1, 1);
			return newClockwisePoints;
		}
		...
		const newInversePolygon = new Polygon(
			newInversePoints.map((point) => point.point),
		);

		// TODO: when enemy added, choose area that enemy is not in...
		const smallerAreaPoints =
			Math.abs(newPolygon.area) <= Math.abs(newInversePolygon.area)
				? newPoints
				: newInversePoints;
	...
	calculateNewPolygonPointsNormal(points: ExtPoint[]): ExtPoint[] {
		return this.calculateNewPolygonPointsWithInnerPoints(
			points,
			this.innerPolygonPointsClockwise.slice(),
			false,
		);
	}
	 ...
	 */
	calculateNewPolygonPointsWithInnerPoints(
		points: ExtPoint[],
		innerPoints: ExtPoint[],
		...
		const processRestOfPoints = (innerPointIndex: number) => {
			...
			for (let i = 0; i < innerPoints.length; i++) {
				const innerPoint1 = innerPoints[innerPointIndex];
				const innerPoint2 = innerPoints[innerPointIndex + 1];
				innerPointIndex =
					innerPointIndex < innerPoints.length - 2 ? innerPointIndex + 1 : 0;

				const firstPointBetween = firstPoint.isBetweenTwoPointsInclusive(
					innerPoint1,
					innerPoint2,
				);
				const lastPointBetween = lastPoint.isBetweenTwoPointsInclusive(
					innerPoint1,
					innerPoint2,
				);
		...
		for (
			let innerPointIndex = 0;
			innerPointIndex < innerPoints.length - 1;
			innerPointIndex++
		) {
			const innerPoint1 = innerPoints[innerPointIndex];
			const innerPoint2 = innerPoints[innerPointIndex + 1];
			...
			const firstAndLastPointsBetween = firstPointBetween && lastPointBetween;
	...
	flattenPoints(points: ExtPoint[]): ExtPoint[] {
		...
		if (
			pts.length >= 3 &&
			pts[0].isBetweenTwoPointsInclusive(pts[pts.length - 2], pts[1])
		) {
	...
	updateNewInnerPoints(newPolygonPoints: ExtPoint[]): void {
		const polygonLines: Line[] =
			GeomUtils.getLinesFromPolygonPoints(newPolygonPoints);
		const innerLines: Line[] = GeomUtils.getLinesFromPolygonPoints(
			this.innerPolygonPointsClockwise,
		);
		...
		outer: for (
			startingInnerLineIndex = 0;
			startingInnerLineIndex < innerLines.length;
			startingInnerLineIndex++
		) {
			const innerLine = innerLines[startingInnerLineIndex];
			for (
				let polygonLinesIndex = 0;
				polygonLinesIndex < polygonLines.length;
				polygonLinesIndex++
			) {
				const polygonLine = polygonLines[polygonLinesIndex];
				if (
					GeomUtils.linesAreEqual(innerLine, polygonLine) ||
					GeomUtils.lineContainsLine(innerLine, polygonLine)
				) {
					break outer;
				}
			}
		...
		const nextIndex = (
			...
			return index < array.length - increment
				? index + increment
				: index + increment - array.length;
		};
		...
		const polygonLinesStartIndexForInsertion = 0;
		...
		for (let i = 0; i < innerLines.length; i++) {
			...
			for (
				...
				if (GeomUtils.lineContainsLineAndNotEqual(innerLine, polygonLine)) {
					lineBisectionToAdd = GeomUtils.subtractLinesWhereLine1ContainsLine2(
						innerLine,
						polygonLine,
					);
		...
		this.scene.debug.infoLines(
			"newInnerLines before adding inner polygon",
			newInnerLines,
		);
		...
		for (let i = 0; i < polygonLines.length; i++) {
			const polygonLinesIndex = nextIndex(
				polygonLines,
				i,
				polygonLinesStartIndexForInsertion,
			);
			const polygonLine = polygonLines[polygonLinesIndex];
		...
		if (polygonLinesToInsert.length > 0) {
			const lastPolygonLineToInsert =
				polygonLinesToInsert[polygonLinesToInsert.length - 1];
			for (
				newInnerLinesInsertionIndex = 0;
				newInnerLinesInsertionIndex < newInnerLines.length;
				newInnerLinesInsertionIndex++
			) {
				const innerLine = newInnerLines[newInnerLinesInsertionIndex];
				if (
					innerLine.x1 === lastPolygonLineToInsert.x2 &&
					innerLine.y1 === lastPolygonLineToInsert.y2
				) {
					break;
				}
			}

			this.scene.debug.info(
				`newInnerLinesInsertionIndex: ${newInnerLinesInsertionIndex}`,
			);
			newInnerLines.splice(
				newInnerLinesInsertionIndex,
				0,
				...polygonLinesToInsert,
			);
		}
		...
		this.innerPolygonPointsClockwise =
			GeomUtils.getPolygonPointsFromLines(newInnerLines);

		this.innerPolygonPointsClockwise = this.flattenPoints(
			this.innerPolygonPointsClockwise,
		);
	}
}
