...
export class Info {
	...
	constructor(scene: QixScene) {
		...
		const pauseButtonOptions = {
			font: Info.TEXT_FONT,
			fill: Info.GAME_TEXT_COLOR_STR,
		};
		const pauseButtonX =
			(config.width as number) - customConfig.margin - Info.PADDING;
		this.pauseButtonText = scene.add.text(
			pauseButtonX,
			this.rectangle.top + Info.PADDING,
			I18nService.t("game.paused"),
			pauseButtonOptions,
		);
		...
		this.pauseButtonText.on("pointerdown", () => {
			...
			this.pauseButtonText.setText(
				this.scene.pauseControl.isPaused()
					? I18nService.t("game.resume")
					: I18nService.t("game.paused"),
			);
		});
	}
	...
	updateGameText() {
		...
		const _player = this.scene.player;
		const _grid = this.scene.grid;
		const _frame = this.scene.grid.frame;
