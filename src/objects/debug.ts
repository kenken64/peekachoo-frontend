...
export class Debug {
	...
	graphics1Y =
		3 * customConfig.margin +
		customConfig.frameHeight +
		customConfig.infoHeight;
	graphics2Y =
		4 * customConfig.margin +
		2 * customConfig.frameHeight +
		customConfig.infoHeight;
	...
	constructor(scene: QixScene) {
		...
		if (customConfig.debug) {
			this.debugTextArea$
				...
				.height(`${customConfig.debugTextAreaHeight}px`);
			this.debugTextArea$.css(
				"font-family",
	...
	highlightPoints(
		...
		const drawPointFunc = (index: string) => {
			const point = points[parseInt(index, 10)];
	...
	infoScroll(): void {
		if (!customConfig.debug) return;
		this.debugTextArea$.scrollTop(
			this.debugTextArea$[0].scrollHeight - this.debugTextArea$.height(),
		);
	}
