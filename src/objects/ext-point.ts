...
export class ExtPoint {
	...
	isLeftOf(p: ExtPoint): boolean {
		return this.x() < p.x();
	}
	isRightOf(p: ExtPoint): boolean {
		return this.x() > p.x();
	}
	isAboveOf(p: ExtPoint): boolean {
		return this.y() < p.y();
	}
	isBelowOf(p: ExtPoint): boolean {
		return this.y() > p.y();
	}
	isLeftAndAboveOf(p: ExtPoint): boolean {
		return this.isLeftOf(p) && this.isAboveOf(p);
	}
	isLeftAndBelowOf(p: ExtPoint): boolean {
		return this.isLeftOf(p) && this.isBelowOf(p);
	}
	isRightAndAboveOf(p: ExtPoint): boolean {
		return this.isRightOf(p) && this.isAboveOf(p);
	}
	isRightAndBelowOf(p: ExtPoint): boolean {
		return this.isRightOf(p) && this.isBelowOf(p);
	}
	isOnSameVerticalAxisOf(p: ExtPoint): boolean {
		return this.point.x === p.x();
	}
	isOnSameHorizontalAxisOf(p: ExtPoint): boolean {
