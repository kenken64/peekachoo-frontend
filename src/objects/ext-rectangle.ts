...
export class ExtRectangle {
	...
	pointOnTopSide(point: Point): boolean {
		return Phaser.Geom.Intersects.PointToLineSegment(
			point,
	...
	nonInteresectingLineOutside(line: Line): boolean {
		return (
			this.pointOutside(new Point(line.x1, line.y1)) &&
			this.pointOutside(new Point(line.x2, line.y2))
		);
	}
}
