...
function resizeCanvas() {
	...
	logger.log(
		`resizeCanvas: windowWidth=${windowWidth}, windowHeight=${windowHeight}`,
	);
	...
	if (windowWidth < 768) {
		...
		logger.log(
			`Mobile detected, scaleX=${scaleX}, scaleY=${scaleY}, scale=${scale}`,
		);
		...
		const offsetY = Math.max(
			0,
			(windowHeight - scaledHeight - mobileHeaderHeight) / 2,
		);
		...
		logger.log(
			`Canvas scaled to ${scaledWidth}x${scaledHeight}, offset: ${offsetX}, ${offsetY}, headerHeight: ${mobileHeaderHeight}`,
		);
	} else {
